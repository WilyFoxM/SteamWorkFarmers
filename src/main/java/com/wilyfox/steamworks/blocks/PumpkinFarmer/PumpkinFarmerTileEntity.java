package com.wilyfox.steamworks.blocks.PumpkinFarmer;
...
public class PumpkinFarmerTileEntity extends LockableTileEntity implements ISidedInventory, ITickableTileEntity {
    ...
    public PumpkinFarmerTileEntity() {
        super(ModTileEntityTypes.PUMPKIN_FARMER.get());
    ...
    public void tick() {
        ...
        for (int x = -2; x <= 2 ; x++) {
            for (int y = -2; y <= 2; y++) {
                ...
                if (world.getBlockState(newPos) == Blocks.AIR.defaultBlockState() && world.getBlockState(newPos.below()).getBlock() == Blocks.FARMLAND && (world.getBlockState(newPos.east()).getBlock() == Blocks.AIR && world.getBlockState(newPos.west()).getBlock() == Blocks.AIR)) {
                    for (int i = 0; i < 4; i++) {
                        if (this.items.get(i).getItem() == Items.PUMPKIN_SEEDS) {
                            this.items.get(i).setCount(this.items.get(i).getCount() - 1);
                            world.setBlockAndUpdate(newPos, Blocks.PUMPKIN_STEM.defaultBlockState());
                            break;
                        }
                    }
                }
        ...
        for (int x = -2 - PUMPKIN_RANGE; x <= 2 + PUMPKIN_RANGE ; x++) {
            for (int y = -2 - PUMPKIN_RANGE; y <= 2 + PUMPKIN_RANGE; y++) {
                ...
                if (pumpkinState.getBlock() == Blocks.PUMPKIN) {
                    for (int i = 0; i < 8; i++) {
                        if (this.items.get(i).getItem() == Items.PUMPKIN && this.items.get(i).getCount() < 64) {
                            ...
                            this.level.addParticle(ParticleTypes.HAPPY_VILLAGER, newPos.getX(), newPos.getY(), newPos.getZ(), 1.0, 1.0, 1.0);
                            this.setItem(i, new ItemStack(Items.PUMPKIN));
                            world.setBlockAndUpdate(newPos, Blocks.AIR.defaultBlockState());
                            break;
                        }
                    }
    ...
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("container.steamworks.pumpkin_farmer");
    }

    @Override
    protected Container createMenu(int id, PlayerInventory playerInventory) {
        return new PumpkinFarmerContainer(id, playerInventory, this, this.fields);
    }
