...
public class BeetrootFarmerTileEntity extends LockableTileEntity implements ISidedInventory, ITickableTileEntity {
    ...
    public BeetrootFarmerTileEntity() {
        super(ModTileEntityTypes.BEETROOT_FARMER.get());
        ...
        for (int x = -2; x <= 2 ; x++) {
            for (int y = -2; y <= 2 ; y++) {
                ...
                if (world.getBlockState(newPos) == Blocks.AIR.defaultBlockState() && world.getBlockState(newPos.below()).getBlock() == Blocks.FARMLAND) {
                    for (int i = 0; i < 4; i++) {
                        if (this.items.get(i).getItem() == Items.BEETROOT_SEEDS) {
                            this.items.get(i).setCount(this.items.get(i).getCount() - 1);
                            world.setBlockAndUpdate(newPos, Blocks.BEETROOTS.defaultBlockState());
                            break;
                        }
                    }
                } else {
                    ...
                    BlockState beetrootState = world.getBlockState(newPos);
                    if (beetrootState == Blocks.BEETROOTS.defaultBlockState().setValue(BlockStateProperties.AGE_3, 3)) {
                        if (this.items.get(4).getCount() >= 64 && this.items.get(5).getCount() >= 64 && this.items.get(6).getCount() >= 64 && this.items.get(7).getCount() >= 64) return;
                        for (int i = 4; i < 8; i++) {
                            if (this.items.get(i).getItem() == Items.BEETROOT && this.items.get(i).getCount() < 64) {
                                ...
                                this.level.addParticle(ParticleTypes.HAPPY_VILLAGER, newPos.getX(), newPos.getY(), newPos.getZ(), 1.0, 1.0, 1.0);
                                this.setItem(i, new ItemStack(Items.BEETROOT));
                                world.setBlockAndUpdate(newPos, Blocks.AIR.defaultBlockState());
                                break;
                            }
                        }
                        ...
                        if (!isFool) {
                            for (int i = 0; i < 4; i++) {
                                if (this.items.get(i).getItem() == Items.BEETROOT_SEEDS && this.items.get(i).getCount() < 64) {
                                    this.items.get(i).setCount(this.items.get(i).getCount() + 1 + rand.nextInt(3));
                                    if (this.items.get(i).getCount() > 64) {
                                        this.items.get(i).setCount(64);
                                    }
                                    break;
                                } else if (this.items.get(i).isEmpty()) {
                                    this.setItem(i, new ItemStack(Items.BEETROOT_SEEDS));
    ...
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("container.steamworks.beetroot_farmer");
    }

    @Override
    protected Container createMenu(int id, PlayerInventory playerInventory) {
        return new BeetrootFarmerContainer(id, playerInventory, this, this.fields);
    }
