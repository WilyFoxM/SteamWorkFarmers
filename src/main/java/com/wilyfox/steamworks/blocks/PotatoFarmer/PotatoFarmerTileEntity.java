package com.wilyfox.steamworks.blocks.PotatoFarmer;
...
public class PotatoFarmerTileEntity extends LockableTileEntity implements ISidedInventory, ITickableTileEntity {
    ...
    public PotatoFarmerTileEntity() {
        super(ModTileEntityTypes.POTATO_FARMER.get());
        ...
        for (int x = -2; x <= 2 ; x++) {
            for (int y = -2; y <= 2 ; y++) {
                ...
                if (world.getBlockState(newPos) == Blocks.AIR.defaultBlockState() && world.getBlockState(newPos.below()).getBlock() == Blocks.FARMLAND) {
                    for (int i = 0; i < 4; i++) {
                        if (this.items.get(i).getItem() == Items.POTATO) {
                            this.items.get(i).setCount(this.items.get(i).getCount() - 1);
                            world.setBlockAndUpdate(newPos, Blocks.POTATOES.defaultBlockState());
                            break;
                        }
                    }
                } else {
                    BlockState potatoState = world.getBlockState(newPos);
                    if (potatoState == Blocks.POTATOES.defaultBlockState().setValue(AGE, 7)) {
                        for (int i = 0; i < 8; i++) {
                            if (this.items.get(i).getItem() == Items.POTATO && this.items.get(i).getCount() < 64) {
                                ...
                                this.level.addParticle(ParticleTypes.HAPPY_VILLAGER, newPos.getX(), newPos.getY(), newPos.getZ(), 1.0, 1.0, 1.0);
                                this.setItem(i, new ItemStack(Items.POTATO));
    ...
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("container.steamworks.potato_farmer");
    }

    @Override
    protected Container createMenu(int id, PlayerInventory playerInventory) {
        return new PotatoFarmerContainer(id, playerInventory, this, this.fields);
    }
