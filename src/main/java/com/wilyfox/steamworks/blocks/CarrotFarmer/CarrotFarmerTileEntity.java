...
public class CarrotFarmerTileEntity extends LockableTileEntity implements ISidedInventory, ITickableTileEntity {
    ...
    public CarrotFarmerTileEntity() {
        super(ModTileEntityTypes.CARROT_FARMER.get());
        ...
        for (int x = -2; x <= 2 ; x++) {
            for (int y = -2; y <= 2 ; y++) {
                ...
                if (world.getBlockState(newPos) == Blocks.AIR.defaultBlockState() && world.getBlockState(newPos.below()).getBlock() == Blocks.FARMLAND) {
                    for (int i = 0; i < 4; i++) {
                        if (this.items.get(i).getItem() == Items.CARROT) {
                            this.items.get(i).setCount(this.items.get(i).getCount() - 1);
                            world.setBlockAndUpdate(newPos, Blocks.CARROTS.defaultBlockState());
                            break;
                        }
                    }
                } else {
                    BlockState carrotState = world.getBlockState(newPos);
                    if (carrotState == Blocks.CARROTS.defaultBlockState().setValue(AGE, 7)) {
                        for (int i = 0; i < 8; i++) {
    ...
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("container.steamworks.carrot_farmer");
    }

    @Override
    protected Container createMenu(int id, PlayerInventory playerInventory) {
        return new CarrotFarmerContainer(id, playerInventory, this, this.fields);
    }
